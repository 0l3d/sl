      ...
      int start_size)
{
    ...
    while (*p != '\0') {
        if (token_count + 1 == size_s) {
            size_s += 1024;
            bufout = realloc(bufout, size_s * sizeof(char *));
        }
        if (*p == '#') {
            break;
        }
                  ...
                  int current_line, char *op_str, int is_op_str)
{
    ...
    if (is_op_str != 1) {
        ...
        case '>':
            ...
            case STRING:
                fprintf(stderr,
                        "Strings cannot bigger than each other. Line: %d\n",
                        current_line);
                break;
            case BOOLEAN:
                fprintf(stderr,
                        "Booleans cannot bigger than each other. Line: %d\n",
                        current_line);
                break;
            case CHAR:
                fprintf(stderr,
                        "Chars cannot bigger than each other. Line: %d\n",
                        current_line);
                break;
            case LONG:
        ...
        case '<':
            ...
            case STRING:
                fprintf(stderr,
                        "Strings cannot less than each other. Line: %d\n",
                        current_line);
                break;
            case BOOLEAN:
                fprintf(stderr,
                        "Booleans cannot less than each other. Line: %d\n",
                        current_line);
                break;
            case CHAR:
                fprintf(stderr,
                        "Chars cannot less than each other. Line: %d\n",
                        current_line);
                break;
            case LONG:
                expression_result.valb = left_side.valh < right_side.valh;
        ...
        if (strcmp(op_str, ">>") == 0) {
            ...
            case DOUBLE:
                fprintf(stderr,
                        "Doubles cannot shift right each other. Line: %d\n",
                        current_line);
                break;
            case STRING:
                fprintf(stderr,
                        "Strings cannot shift right each other. Line: %d\n",
                        current_line);
                break;
            case BOOLEAN:
                fprintf(stderr,
                        "Booleans cannot shift right each other. Line: %d\n",
                        current_line);
                break;
            case CHAR:
                fprintf(stderr,
                        "Chars cannot shift right each other. Line: %d\n",
                        current_line);
                break;
            case LONG:
                expression_result.valh = left_side.valh >> right_side.valh;
            ...
            case DOUBLE:
                fprintf(stderr,
                        "Doubles cannot shift left each other. Line: %d\n",
                        current_line);
                break;
            case STRING:
                fprintf(stderr,
                        "Strings cannot shift left each other. Line: %d\n",
                        current_line);
                break;
            case BOOLEAN:
                fprintf(stderr,
                        "Booleans cannot shift left each other. Line: %d\n",
                        current_line);
                break;
            case CHAR:
                fprintf(stderr,
                        "Chars cannot shift left each other. Line: %d\n",
                        current_line);
                break;
            case LONG:
                expression_result.valh = left_side.valh << right_side.valh;
            ...
            case STRING:
                fprintf(stderr,
                        "Strings cannot conditional and each other. Line: %d\n",
                        current_line);
                break;
            case BOOLEAN:
                expression_result.valb = left_side.valb && right_side.valb;
            ...
            case STRING:
                fprintf(stderr,
                        "Strings cannot conditional or each other. Line: %d\n",
                        current_line);
                break;
            case BOOLEAN:
                expression_result.valb = left_side.valb || right_side.valb;
...
prec_priority(char op, char *op_str, int is_op_str)
{
    if (is_op_str == 1) {
        ...
        else if (strcmp(op_str, "equ") == 0 || strcmp(op_str, "eql") == 0 ||
                 strcmp(op_str, "neq") == 0 || strcmp(op_str, "eqg") == 0) {
            return 2;
        }
                           ...
                           int max_tokens, int current_line)
{
    ...
    while (current_token < max_tokens) {
        ...
        if (op_prec != 0) {
            ...
            if (op_prec <= prec_priority(tree.op, tree.op_str, tree.is_op_str)
                && op_prec != 0) {
                tree.op = op;
                         ...
                         int *current_token, int max_tokens, int current_line)
{
    ...
    struct SL_Math_Splitter tree =
        expression_parser_splitter(expression, *current_token, max_tokens,
                                   current_line);
    if (tree.op == 0) {
    ...
    if (operator_checker(expression, right_pos_start, right_pos_end) != 0) {
        right = expression_parser_solver(code_s, expression, &right_pos_start,
    ...
    result =
        expression_solver(left, tree.op, right, current_line, tree.op_str,
                          tree.is_op_str);
    *current_token = max_tokens;
    return result;
}
                ...
                int max_tokens, int current_line)
{
    ...
    while (current_token != NULL && *current_token < max_tokens) {
        if (strcmp(tokens[*current_token], ",") == 0) {
            if (tokens[(*current_token) - 2] != NULL &&
                strcmp(tokens[(*current_token) - 2], "=") != 0) {
        ...
        if (strcmp(tokens[*current_token], "=") == 0) {
            ...
            int value_end = 0;
            int out =
                multi_variable_checker(tokens, *current_token, max_tokens);
            if (out != -1) {
                value_end = out;
            }
            else {
            ...
            struct SL_Variable result =
                expression_parser_solver(code_s, tokens, &equal_start,
                                         value_end, current_line);
    ...
    while (current_token != NULL && *current_token < max_tokens) {
        ...
        if (is_has_token("=", tokens, *current_token, last_pos) == 1) {
            struct SL_Variable out_var =
                assignment_parser(code_s, tokens, current_token, last_pos,
                                  current_line);
        }
        *current_token = max_tokens;
    }
             ...
             int max_tokens, int current_line)
{
    ...
    sl_then_finder(tokens, *current_token, max_tokens, &if_start_pos);
    expr =
        expression_parser_solver(code_s, tokens, current_token,
    ...
    for (int current_token = 0; current_token < code_s.token_count;
        ...
        if (strcmp(code_s.code[current_token], "var") == 0) {
            for (int i = current_token; i < code_s.token_count; i++) {
                if (code_s.code[i][0] == '=') {
                    i++;
                    end = find_maxt_expr(code_s.code, i, code_s.token_count);
                    break;
                }
            }
            ...
            struct SL_Variable_Creator vars =
                variable_parser(code_s, code_s.code, &current_token, end,
                                0);
            ...
            struct SL_Variable out_boolean =
                sl_if_parser(code_s, code_s.code, &current_token,
                             current_token,
